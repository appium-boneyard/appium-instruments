...
class Instruments extends EventEmitter {
  // simpler constructor
  static async quickInstrument (opts) {
    opts = _.clone(opts);
  ...
  async launchOnce () {
    ...
    let actOnStderr = (output) => {
      if (this.launchTimeout.afterSimLaunch && output && output.match(/CLTilesManagerClient: initialize/)) {
        this.addSocketConnectTimer(this.launchTimeout.afterSimLaunch, 'afterLaunch');
      }
    ...
    this.addSocketConnectTimer(this.launchTimeout.global, 'global');
  ...
  addSocketConnectTimer(delay, type) {
