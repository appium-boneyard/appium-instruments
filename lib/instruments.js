...
Instruments.prototype.launchAndKill = function(condition, cb) {
  ...
  this.configure(function(err) {
    ...
    diedTooYoung.on('exit', function(code, singal) {
      logger.debug("instruments exited with code(" + code + ") due to singal: " + singal);
      diedTooYoungExited = true;
    });
    ...
    var timelyCauseOfDeath = function() {
      ...
      if (condition()) {
        ...
        if (diedTooYoungExited) { //relaunch it
          Instruments.killAllSim();
          // waiting a bit before restart
          setTimeout(function() {
            this.launchAndKill(condition, cb);
          }.bind(this), 5000);
        } else {
    ...
    process.nextTick(timelyCauseOfDeath);  // why nextTick()???
