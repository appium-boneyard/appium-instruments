...
async function getXcodeTraceTemplatePath () {
  ...
  let instrumentsAppPath = path.resolve(xCodePath, '..', '..', 'Contents/Applications/Instruments.app');
  let traceTemplatePath = path.resolve(instrumentsAppPath,
...
async function killAllSimulators () {
  ...
  if (version >= "6") {
    ...
    log.debug("Killing all iPhoneSimulator");
    try {
      await exec('pkill', ['-9', '-f', 'iPhoneSimulator']);
    } catch (ign) {}
  }
}
